using TreeWidthSolver: open_neighbors, closed_neighbors, is_clique

@testset "BitGraphs" begin
...
@testset "neighbors" begin
    ...
    bg = BitGraph(g)
    for i in 1:10
        @test neighbors(bg, i) == bmask(LongLongUInt{1}, g.fadjlist[i])
    end
    sub_bg = induced_subgraph(bg, bmask(LongLongUInt{1}, 1:5))
    sub_g = induced_subgraph(g, 1:5)[1]

    for i in 1:5
        @test neighbors(sub_bg, i) == bmask(LongLongUInt{1}, sub_g.fadjlist[i])
    end
...
@testset "connected component" begin
    ...
    @test is_connected(bg, 1, 2)
    @test !is_connected(bg, 1, 4)
